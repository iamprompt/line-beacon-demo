...
if (command === 'create') {
  ...
  const result = await asyncExec(`npx wrangler d1 migrations create ${database} ${migrationName}`)
  ...
  // SQL filename regex
  const regexFilePath = /\/.*.sql/g
  const resultText = result.trim().split('\n').join('')

  const migrationPath = resultText.match(regexFilePath)?.[0]
  if (!migrationPath) {
  ...
  await asyncExec(
    `pnpm prisma migrate diff --script --from-local-d1 --to-schema-datamodel ${
...
if (command === 'apply' || command === 'create') {
  ...
  await asyncExec(`npx wrangler d1 migrations apply ${database} ${args.remote ? '--remote' : '--local'}`)

  s.stop('Applying migrations')

  s.start('Generating Prisma client')

  await asyncExec(`npx prisma generate ${args.schema ? `--schema ${args.schema}` : ''}`)
